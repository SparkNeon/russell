...
def gen_passwords(x):
    ...
    return [''.join(random.choices(string.ascii_uppercase + string.digits, k=40)) for _ in range (x)]
...
auction.to_csv("auction_data", sep='\t')
...
wish_user_id = random.sample(user_id_list, k=200)
wish_auction_id = random.sample(auction_id_list, k=200)
